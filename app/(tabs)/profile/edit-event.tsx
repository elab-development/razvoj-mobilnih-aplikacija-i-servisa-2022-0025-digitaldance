...
export default function EditEventScreen() {
  ...
  useEffect(() => {
    getEventById(id).then((event) => {
      if (event) {
        setCoverUri(event.cover_image_url);
        ...
        setDateDraft(eventDate);
        setTimeDraft(eventDate);

        const city = event.city ?? "";
        const coordinate = { latitude: event.location_lat ?? DEFAULT_REGION.latitude, longitude: event.location_lng ?? DEFAULT_REGION.longitude };
        setPlace({ address: city, city, ...coordinate });
  ...
  const handleSave = async () => {
    ...
    let coverUrl: string | null = coverUri;
    ...
    Alert.alert("Successfully edited event", undefined, [{ text: "OK", onPress: () => router.back() }]);
  };
    ...
    Alert.alert("Delete event", "Are you sure you want to delete this event?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          Alert.alert("Successfully deleted event", undefined, [{ text: "OK", onPress: () => router.back() }]);
        },
      },
    ]);
  };
